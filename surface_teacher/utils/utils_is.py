...
def train(model, num_epoch, device, criterion, weight_path, lr, dataloader_train, dataloader_val, \
    ...
    for epoch in range(num_epoch):
        ...
        for inputs, labels in pbar_train:
            ...
            imgs = [inputs[1][0].to(device).float(), inputs[1][1].to(device).float()]
            labels = labels.to(device).float()

            num_iters_train += 1
            #print("here 0")
            
            y_pred, out1, out2, att_w1, att_w2, att_w11, att_w22 = model(x, imgs)
            # print(">>>>>>>>>>>>>> ", y_pred.shape, labels.shape)
            # print(labels)

            loss= criterion(y_pred, labels) + criterion(out1, labels) + criterion(out2, labels)
            # print(">>>>>>>>>>>>>>>>>>>>>>>>>>")
            # print(att_w11.shape) #surface
            # print(att_w1.shape) #mvnet
            
            loss_dis = dis_loss(att_w11, att_w1) + dis_loss(att_w22, att_w2)
            ...
            pbar_train.set_description(f"Loss: {epoch_train_loss.item()/num_iters_train:.4f}, dis_loss: {epoch_loss_dis.item()/num_iters_train:.4f}, acc: {epoch_train_acc.item()/num_iters_train:.4f}")
        ...
        #print("epoch", (epoch+1))
        print("train_acc:", train_accuracy.item(), "train_loss:", train_loss.item(), "loss_dis: ", (epoch_loss_dis/num_iters_train).item())
