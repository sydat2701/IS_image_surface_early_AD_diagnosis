...
class PatchEmbeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x).view(B, L, -1)
        #print(x.shape, y.shape, positions.shape)
        # x = torch.cat([x, y], dim=2) + positions
...
class LocalPatch(nn.Module):
    def __init__(self, n_ins):
        super(LocalPatch, self).__init__()
        self.conv1d = nn.Sequential(nn.Conv1d(n_ins, 8, kernel_size=3), nn.ReLU(inplace=True), nn.Dropout(0.4), \
                                    nn.Conv1d(8, 16, kernel_size=3, stride=2), nn.ReLU(inplace=True), nn.Dropout(0.4))
        self.down_channel = nn.Sequential(nn.Conv1d(in_channels=1200, out_channels=60, kernel_size=1), \
                                          nn.ReLU(inplace=True))
        
    def forward(self, x):
        B, C, P, V = x.shape
        x = torch.permute(x, (0, 2, 3, 1)) #(B, P, V, C)
        x = x.contiguous().view(B*P, V, C).transpose(1,2)
        x = self.conv1d(x)
        x = x.view(B, P, -1).transpose(1,2)
...
class AttentionModel(nn.Module):
    def __init__(self, dims, depth, heads, num_patches=20, num_classes=1, num_channels=3, num_vertices=153, dropout=0.1, branches=[], activation='sigmoid', device = 'cuda'):
        ...
        self.patch_embeddz = PatchEmbeddings(self.num_patches, self.dims, n_ins=1).to(device)
        ...
        self.trans_cross_att_block3 = nn.ModuleList([TransformerBlock(self.dims, self.heads, mlp_ratio=4, dropout_ratio=self.dropout).to(device) for i in range(depth[1])])
        self.trans_cross_att_block4 = nn.ModuleList([TransformerBlock(self.dims, self.heads, mlp_ratio=4, dropout_ratio=self.dropout).to(device) for i in range(depth[1])])
        self.trans_cross_att_block5 = nn.ModuleList([TransformerBlock(self.dims, self.heads, mlp_ratio=4, dropout_ratio=self.dropout).to(device) for i in range(depth[1])])
        self.trans_cross_att_block6 = nn.ModuleList([TransformerBlock(self.dims, self.heads, mlp_ratio=4, dropout_ratio=self.dropout).to(device) for i in range(depth[1])])

        self.fc1 = nn.Linear(640*192*6, dims)
        self.norm1 = nn.LayerNorm(dims, eps=1e-6)
        self.fc2 = nn.Linear(dims, self.num_classes)
        self.sig = nn.Sigmoid()

        # self.localx = LocalPatch(n_ins=3)
        # self.localy = LocalPatch(n_ins=1)
        # self.localz = LocalPatch(n_ins=1)
    ...
    def forward(self, x):
        ...
        # print(">>>>>>>>>>>>>>>>>>")
        # print(x.shape) #[8, 4, 640, 153]
        #x = nn.GaussianNoise(1)(x)
        x = x + (1**0.5)*torch.randn_like(x)
        x = [x[:, :3, :], x[: , 3, :].unsqueeze(1), x[: , 4, :].unsqueeze(1)]
        # print(x[0].shape, x[1].shape)

        # feat_localx = self.localx(x[0])
        # feat_localy = self.localy(x[1])
        # feat_localz = self.localz(x[2])
        # print(feat_localx.shape, feat_localy.shape) #[8, 640, 64]
        # exit()       
        
        x = [self.patch_embeddx(x[0]), self.patch_embeddy(x[1]), self.patch_embeddz(x[2])]
        # x = [torch.cat([x[0], feat_localx], dim=1), torch.cat([x[1], feat_localy], dim=1)]
        #print(x[0].shape)
        
        for i in range(self.depth[0]):
            # x = [self.trans_att_block(x[0]), self.trans_att_block(x[1])]
            x = [self.trans_att[i](x[0]), self.trans_att[i](x[1]), self.trans_att[i](x[2])]
            
        if len(self.depth) > 1:
            for i in range(self.depth[1]):
                x = [self.trans_cross_att_block1[i](x[0], x[1]),
                     self.trans_cross_att_block2[i](x[1], x[0]),
                     self.trans_cross_att_block3[i](x[1], x[2]),
                     self.trans_cross_att_block4[i](x[2], x[1]),
                     self.trans_cross_att_block5[i](x[0], x[2]),
                     self.trans_cross_att_block6[i](x[2], x[0])]
                     
        # print("*************************")
        # print("///////////// ", x[0].shape, x[0].shape)
        
        # print(">>>>>>>>>> ", x[0].shape) #[8, 640, 192]
        # exit()
        x = [self.norm(x[0]), self.norm(x[1]), self.norm(x[2]), self.norm(x[3]), self.norm(x[4]), self.norm(x[5])]
        x = [nn.Flatten()(y) for y in x]
        # print("///////////// ", x[0].shape, x[1].shape) #[8, 122880]
        # exit()
        ...
        if len(x) > 1:
            ...
            x = x[0]
        # x = torch.cat([x, feat_localx, feat_localy], dim=1)
