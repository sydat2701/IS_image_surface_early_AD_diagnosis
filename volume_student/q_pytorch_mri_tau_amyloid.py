...
class AttentionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.depth[0]):
            ...
            outz, fused_attz, fuse_v_z = self.trans_att[i](x[2])
            x = [outx, outy, outz]
        ...
        if len(self.depth) > 1:
            for i in range(self.depth[1]):
                x = [self.trans_cross_att_block1[i](x[0], x[1])[0],
                     ...
                     self.trans_cross_att_block3[i](x[1], x[2])[0],
                     self.trans_cross_att_block4[i](x[2], x[1])[0],
                     self.trans_cross_att_block5[i](x[0], x[2])[0],
                     self.trans_cross_att_block6[i](x[2], x[0])[0]]
        # print("::::::::::::::::: ", len(x))
        # print(x)
        ...
        return x, fused_attx, fuse_v_x, fused_atty, fuse_v_y, fused_attz, fuse_v_z
