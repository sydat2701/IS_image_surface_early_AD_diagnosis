...
teacher_path = './teacher/teacher_weights/cn_ad'
...
def train(num_epoch, device, criterion, weight_path, lr, dataloader_train, dataloader_val, \
    ...
    from extract_net_extend_2modal import MVNet
    ...
    for epoch in range(num_epoch):
        ...
        for inputs, labels in dataloader_train:
            ...
            y_pred, y_pred_mri, y_pred_fdg, fuse_att_surfx, fuse_v_surfx, fuse_att_surfy, \
            fuse_v_surfy, att_weight_imgx, att_value_imgx, att_weight_imgy, att_value_imgy = model(mri, fdg, x)
            ...
            total_loss = loss + fuse_att_lossx + fuse_v_lossx + fuse_att_lossy + fuse_v_lossy
...
def main():
    ...
    weight_path = './weights'.format(seed)
    ...
    data_path = '../Data_ADNI1_ANTS'
    batch_size = 4
    lr = 0.0003
    tasks = ['cn_ad']

    X, y = np.load('./surface/ADNI1/data_fsaverage_nopvc_ico2.npy').astype('float32'), np.load('./surface/ADNI1/labels.npy')
